...
from tensorflow.keras.layers import Dense, Dropout, LSTM, SimpleRNN, Bidirectional, LeakyReLU, GRU, \
    GlobalAveragePooling1D, GlobalMaxPooling1D, Concatenate, Input, Conv1D, MaxPooling1D, Flatten, Maximum, Minimum, \
    Add, Average, TimeDistributed, Flatten, Masking
...
class STM_Train_BLSTM:
    def __init__(self, vectorizer, data_file, name="", vector_length=64, max_seq_length=50, max_code_stmt_length=40,
                 batch_size=64):
    ...
    def init_combined_model(self):
        ...
        model_3 = Bidirectional(LSTM(lsm_unit, return_sequences=True), input_shape=bw_cdg_ctx_input_shape)(
            model_input_3)
        model_3 = GlobalMaxPooling1D()(model_3)

        model_input_5 = Input(shape=bw_ddg_ctx_input_shape)
        model_5 = Bidirectional(LSTM(lsm_unit, return_sequences=True), input_shape=bw_ddg_ctx_input_shape)(
            model_input_5)
        model_5 = GlobalMaxPooling1D()(model_5)

        model_input_7 = Input(shape=operation_ctx_input_shape)
        model_7 = Bidirectional(LSTM(lsm_unit // 2, return_sequences=True), input_shape=operation_ctx_input_shape)(
            model_input_7)
    ...
    def train(self, epochs=200):
        ...
        mcp_save = ModelCheckpoint("models/best_model." + self.name + "_model.hdf5", save_best_only=True,
                                   monitor="accuracy", mode='max', verbose=2)
        reduce_lr_loss = ReduceLROnPlateau(monitor='loss', factor=0.1, patience=10, min_delta=1e-4, mode='min',
                                           verbose=1)
