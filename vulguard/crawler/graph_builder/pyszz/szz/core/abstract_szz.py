...
from config import REPOSITORY_COMMITS_DIR
from file_manager import join_path, write_file
...
class AbstractSZZ(ABC):
    ...
    def __init__(self, repo_full_name: str, repo_dir: str):
        """
        ...
        self._repository = Repo(self._repository_dir)
        self._pydriller_repo = PyDrillerGitRepo(self.repository_path)

    def __del__(self):
        log.debug("cleanup objects...")
        self.__cleanup_repo()
    ...
    @abstractmethod
    def find_vcc(self, fix_commit_hash: str, impacted_files: List['ImpactedFile'], **kwargs) -> Set[Commit]:
        """
    ...
    def get_impacted_files(self, fix_commit_hash: str,
        ...
        fix_commit = self._pydriller_repo.get_commit(fix_commit_hash)
    ...
    def _blame(self, rev: str,
        ...
        for entry in self.repository.blame_incremental(**kwargs, rev=rev, L=mod_line_ranges, file=file_path):
            ...
            for line_num in entry.orig_linenos:
                ...
                # TODO: mapping line number in current revision to corresponding line number in blamed commit
                line_str = source_file_content.splitlines()[line_num - 1].strip()
                if line_str == "":
                    continue
                b_data = BlameData(entry.commit, line_num, line_str, entry.orig_path)

                related_source_file_path = join_path(REPOSITORY_COMMITS_DIR, self.repo_full_name, b_data.commit.hexsha,
                                                     b_data.file_path)
                write_file(related_source_file_path, source_file_content, skip_if_existed=True)

                if skip_comments and self._is_comment(line_num, related_source_file_path):
                    # log.info(f"skip comment line ({line_num}): {line_str}")
                    continue

                # log.info(f"{file_path}#L{entry.linenos} - {b_data}")
    ...
    def _is_comment(self, line_num: int, source_file_path: str) -> bool:
        """
        ...
        :returns bool
        """
        comment_ranges = parse_comments(source_file_path)
