...
def patched_open_repository(self):
    self._repo = Repo(self.path)
    # Skipping self._repo.config_writer() to avoid lock

PyDrillerGitRepo._open_repository = patched_open_repository
...
class VSZZ(AbstractSZZ):
    ...
    Bao, L., Xia, X., Hassan, A. E., & Yang, X. (2022, May). V-SZZ: automatic identification of version ranges affected by CVE vulnerabilities. 
    In Proceedings of the 44th International Conference on Software Engineering (pp. 2352-2364).
    ...
    * ignore_revs_file_path
    ...
    def __init__(self, repo_full_name: str, repo_url: str, repos_dir: str = None, ast_map_path = None, logger=None):
        super().__init__(repo_full_name, repo_url, repos_dir, logger)
                ...
                for entry in blame_data:
                    ...
                    while True:
                        if imp_file.file_path.endswith(".java"):
                            ...
                            previous_commits.append((blame_result.commit, blame_result.line_num, blame_result.line_str, change_type))
                        else:
