...
class AbstractSZZ(ABC):
    ...
    def __init__(self, repo_full_name: str, repo_url: str, repos_dir: str = None, logger: log.Logger = None):
        """
        ...
        The init method also sets the default_ignore_regex for modified lines.
        ...
        :param log.Logger logger: logger instance to be used, if not provided, a default logger will be created
        """
        self._repository = None

        # Set up the logger, either passed in or create a default one
        self.logger = logger
        ...
        if not os.path.isdir(self._repository_path):
            if repos_dir:
                ...
                if os.path.isdir(repo_dir):
                    ...
                    self.logger.error(f'unable to find local repository path: {repo_dir}')
                    exit(-4)
            else:
                self.logger.info(f"Cloning repository {repo_full_name}...")
                Repo.clone_from(url=repo_url, to_path=self._repository_path)

        self._repository = Repo(self._repository_path)
        self._repository.git.custom_environment(GIT_OPTIONAL_LOCKS='0')

    def __del__(self):
        self.logger.info("cleanup objects...")
        self.__cleanup_repo()
    ...
    def get_impacted_files(self, fix_commit_hash: str,
        ...
        """
         ...
         impacted file path and modified line ranges. As default behavior, all deleted lines in the diff which
        ...
        try:
            ...
            for mod in fix_commit.modifications:
                ...
                if file_ext_to_parse:
                    ...
                    if len(ext) < 2 or (len(ext) > 1 and ext[1].lower() not in file_ext_to_parse):
                        self.logger.info(f"skip file: {mod.filename}")
                        continue
            ...
            self.logger.error(f"Repo: {self.repository_path} - Missing: {fix_commit_hash}")
        # self.logger.info(impacted_files)
        ...
        for entry in self.repository.blame_incremental(**kwargs, rev=rev, L=mod_line_ranges, file=file_path):
            ...
            for line_num in entry.orig_linenos:
                ...
                if skip_comments and self._is_comment(line_num, source_file_content, ntpath.basename(b_data.file_path)):
                    self.logger.info(f"skip comment line ({line_num}): {line_str}")
                    continue

                self.logger.info(b_data)
